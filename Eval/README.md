...
This folder is the **report card** for those jobs: ~100 test questions with known-correct answers, run against the real app code and scored automatically. It measures how often the bouncer makes the right call, how often the librarian grabs the right page, and — via an online judge — whether the AI's final answer stays true to the pages it was handed. It re-runs with one command, so we can prove a change actually helped (or, honestly, didn't).
...
### Groundedness — the *writer* (online, secondary metric)

The bouncer and librarian are measured above. The last box — does the AI's *answer* stay faithful to the pages it was handed? — is measured by an LLM-as-judge: for each in-domain question, Claude answers **only from the retrieved chunks**, then Claude judges whether every factual claim is supported by those chunks ([`groundedness_judge.py`](groundedness_judge.py)).

**Faithfulness 0.83** (43/52 answers fully grounded) · **mean relevance 4.63/5** (n=52, `claude-opus-5`).

The 9 ungrounded answers are the useful part, and they trace straight back to retrieval: most add a *plausible, correct* CBIT fact the model knew but that wasn't in the retrieved chunks — e.g. "hold the competing response until the urge fades" (the how-long detail lives in a chunk that wasn't in the top-4 for per-tic questions), or expanding the "CBIT" acronym on the one question whose defining chunk retrieval *missed* (it ranked 10th). So **improving retrieval would directly lift faithfulness** — the three metrics are one system. This is an **online, non-deterministic, subjective** signal (network + API key; the generator and judge share a model family, which can be lenient), reported alongside — not inside — the offline reproducible gate. Reproduce: `./Eval/run_groundedness.sh`.
...
| `groundedness_judge.py` / `run_groundedness.sh` | Online LLM-as-judge for answer faithfulness (needs `ANTHROPIC_API_KEY`; secondary metric). |
| `results/groundedness.json` | Per-answer verdicts + faithfulness summary (generated by the judge). |
...
- **Groundedness is online & subjective** — the faithfulness judge (0.83, above) needs network + an API key, is non-deterministic on a hosted model, and uses the same model family to generate and judge (possible leniency). It is reported alongside the offline gate, never inside it, and is not part of CI.
